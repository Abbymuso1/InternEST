...
export const _jobs = [...Array(12)].map((_, index) => {
  ...
  const level = (index % 4 && 'intern/student') || 'No experience';
...
export const _jobsByCompanies = [...Array(5)].map((_, index) => ({
  id: _mock.id(index),
  name: _mock.companyName(index),
  logo: _mock.image.company(index),
  totalJobs: 10 + index,
...
export const _jobsByCategories = [...Array(4)].map((_, index) => ({
  id: _mock.id(index),
  icon: ICONS[index],
  name: _mock.jobTitle(index),
  totalJobs: _mock.number.nativeS(index),
}));
